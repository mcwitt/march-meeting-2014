  ...
  \begin{columns}
    ...
    \begin{tikzpicture}[scale=1.3]
      ...
      \begin{scope}[even odd rule]
        \clip\egg;
        \filldraw[thick,fill=blue!20] (0,-2) circle (2.2);
      \end{scope}
      \filldraw[thick,fill=green!20] (0,-0.95) circle (0.75) node {P};
      \path (0,2) node[above] {NP};
      \path (0,0.2) node[below] (npi) {NPI};
      \path (0,1) node {NPC};
      \draw[thick] \egg;
    \end{tikzpicture}
    \begin{tikzpicture}[red,overlay,thick,>=stealth]
      \path[->]<2> (inpi) edge [out=0,in=-160] (npi.west);
    \end{tikzpicture}
  \end{columns}
  \transdissolve<2>[duration=0.2]
\end{frame}
